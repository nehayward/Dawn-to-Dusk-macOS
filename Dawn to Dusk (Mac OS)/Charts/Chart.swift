...
@IBDesignable
open class Chart: NSControl {

    // MARK: Options

    @IBInspectable
    open var identifiera: String?
    ...
    */
    open var labelFont: NSFont? = NSFont.systemFont(ofSize: 12)

    /**
    Font used for the labels.
    */
    @IBInspectable
    open var labelColor: NSColor = NSColor.black

    /**
    Color for the axes.
    */
    @IBInspectable
    open var axesColor: NSColor = NSColor.gray.withAlphaComponent(0.3)

    /**
    Color for the grid.
    */
    @IBInspectable
    open var gridColor: NSColor = NSColor.gray.withAlphaComponent(0.3)
    ...
    */
    open var highlightLineColor = NSColor.gray
    ...
    */
    ...
    override public init(frame: CGRect) {
        ...
        let trackingArea = NSTrackingArea(rect: frame , options: [NSTrackingAreaOptions.activeInKeyWindow,NSTrackingAreaOptions.mouseEnteredAndExited, NSTrackingAreaOptions.mouseMoved], owner: self, userInfo: nil)
    ...
    */
    ...
    fileprivate func drawIBPlaceholder() {
        let placeholder = NSView(frame: self.frame)
        placeholder.layer?.backgroundColor = NSColor(red: 0.93, green: 0.93, blue: 0.93, alpha: 1).cgColor
        let label = NSTextField()
        label.stringValue = "Chart"
  ...
  fileprivate func drawBezierArea(_ xValues: Array<Float>, yValues: Array<Float>, seriesIndex: Int) {
    ...
    fileprivate func drawLabelsAndGridOnXAxis() {
        ...
        scaled.enumerated().forEach { (i, value) in
            ...
            // Add label
            let label = NSTextField(frame: CGRect(x: x, y: 0, width: 0, height: 0))
            ...
            label.stringValue = xLabelsFormatter(i, labels[i])
            label.textColor = labelColor
...
//            label.textAlignment = xLabelsTextAlignment
    ...
    fileprivate func drawLabelsAndGridOnYAxis() {
        ...
        scaled.enumerated().forEach { (i, value) in
            ...
            let label = NSTextField(frame: CGRect(x: padding, y: y, width: 0, height: 0))
            label.font = labelFont
            label.stringValue = yLabelsFormatter(i, labels[i])
            label.textColor = labelColor
            label.sizeToFit()
          
            // Set Label Look Alike
    ...
    func handleTouchEvents(_ touch: NSPoint, event: NSEvent!) {
   ...
   open override func mouseEntered(with event: NSEvent) {
    ...
    Swift.print(event.locationInWindow)
    let locationInView = convert(event.locationInWindow, from: nil)
    

      handleTouchEvents(locationInView, event: event)
...
//
//    override open func touchesBegan(_ touches: Set<NSTouch>, with event: NSEvent?) {
//        handleTouchEvents(touches, event: event)
//    }
//
//    override open func touchesEnded(_ touches: Set<NSTouch>, with event: NSEvent?) {
//        handleTouchEvents(touches, event: event)
...
//    }
//
//    override open func touchesMoved(_ touches: Set<NSTouch>, with event: NSEvent?) {
//        handleTouchEvents(touches, event: event)
//    }
//  
//  
